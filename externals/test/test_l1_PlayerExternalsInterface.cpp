...
TEST(FakePlayerIarmInterface, ReturnsFalseWhenGstElementIsNull) {
    ...
    EXPECT_NO_THROW({
        ...
        bool ret = EXPECT_NO_THROW(player.IsSourceUHD()), retVal = player.IsSourceUHD();
...
TEST(FakePlayerIarmInterface, SetGstElementWithValidPointer) {
    ...
    EXPECT_NO_THROW({
        ...
        // Retrieve the internally set m_gstElement via pointer arithmetic.
        size_t offset = offsetof(PlayerExternalsInterfaceBase, m_gstElement);
        GstElement* internalPtr = *(reinterpret_cast<GstElement**>(reinterpret_cast<char*>(&player) + offset));
        std::cout << "After invocation, internal m_gstElement value is: " << internalPtr << std::endl;
      
        // Verify the internal pointer is updated to validElement1.
        EXPECT_EQ(internalPtr, pValidElement1);
    });
...
TEST(FakePlayerIarmInterface, SetGstElementWithNullPointer) {
    ...
    EXPECT_NO_THROW({
        ...
        // Use a null pointer for GstElement.
        GstElement* pNullElement = nullptr;
        std::cout << "Invoking setGstElement with nullptr" << std::endl;
        EXPECT_NO_THROW(player.setGstElement(pNullElement));
        ...
        // Verify the internal pointer is set to nullptr.
        EXPECT_EQ(internalPtr, pNullElement);
    });
...
TEST(FakePlayerIarmInterface, SetGstElementWithDifferentValidPointer) {
    ...
    EXPECT_NO_THROW({
        ...
        // Create the first valid GstElement and set it.
        ...
        size_t offset = offsetof(PlayerExternalsInterfaceBase, m_gstElement);
        GstElement* internalPtr = *(reinterpret_cast<GstElement**>(reinterpret_cast<char*>(&player) + offset));
        std::cout << "Internal m_gstElement after first set: " << internalPtr << std::endl;
        EXPECT_EQ(internalPtr, pFirstElement);
      
        // Create a different valid GstElement and update the pointer.
        ...
        // Retrieve the updated pointer.
        internalPtr = *(reinterpret_cast<GstElement**>(reinterpret_cast<char*>(&player) + offset));
        std::cout << "After updating, internal m_gstElement value is: " << internalPtr << std::endl;
      
        // Verify the internal pointer is updated to the second valid element.
        EXPECT_EQ(internalPtr, pSecondElement);
    });
...
TEST(FakePlayerIarmInterface, NormalObjectDestruction) {
    ...
    EXPECT_NO_THROW({
        ...
        fakePlayer->m_gstElement = reinterpret_cast<GstElement*>(0x1234);
        std::cout << "m_gstElement set to: " << fakePlayer->m_gstElement << std::endl;
        
        // Assign valid member values.
        fakePlayer->m_isHDCPEnabled = true;
        fakePlayer->m_displayWidth = 1920;
        fakePlayer->m_displayHeight = 1080;
        fakePlayer->m_sourceWidth = 1280;
        fakePlayer->m_sourceHeight = 720;
        
        std::cout << "m_isHDCPEnabled set to: " << fakePlayer->m_isHDCPEnabled << std::endl;
        std::cout << "m_displayWidth set to: " << fakePlayer->m_displayWidth << std::endl;
        std::cout << "m_displayHeight set to: " << fakePlayer->m_displayHeight << std::endl;
        std::cout << "m_sourceWidth set to: " << fakePlayer->m_sourceWidth << std::endl;
        std::cout << "m_sourceHeight set to: " << fakePlayer->m_sourceHeight << std::endl;
