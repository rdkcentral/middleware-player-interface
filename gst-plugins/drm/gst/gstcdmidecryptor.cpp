		...
		if (socInterface && socInterface->IsTransformCapsRequired())
		{
			if (direction == GST_PAD_SINK && !gst_caps_is_empty(transformedCaps))
			{
				...
				if (OCDMGstTransformCaps)
				{
					...
					// track whether the SVP memory feature actually made it into sinkCaps, to correlate with allocator crashes
					gboolean hasSecureFeature = platformSecureMemFeature && platformSecureMemFeature[0] && strstr(caps_after, platformSecureMemFeature);
					GST_INFO_OBJECT(cdmidecryptor, "surya - OCDMGstTransformCaps ret=%d hasSecureFeature=%d transformedCaps after: %s", ret, hasSecureFeature, caps_after);
					g_free(caps_after);
				}
				else
				{
		...
		GstBaseTransform * trans, GstBuffer * buffer)
{
	...
	// log allocation-relevant parameters right before the vendor SVP decrypt/allocate call, to correlate with any downstream crash
	GST_INFO_OBJECT(cdmidecryptor, "surya - pre-decrypt: mediaType=%d bufSize=%" G_GSIZE_FORMAT " ivSize=%u subSampleCount=%u sinkCaps=%" GST_PTR_FORMAT,
					(int)cdmidecryptor->mediaType, gst_buffer_get_size(buffer), ivSize, subSampleCount, cdmidecryptor->sinkCaps);
	g_print("surya - pre-decrypt: mediaType=%d bufSize=%" G_GSIZE_FORMAT " ivSize=%u subSampleCount=%u sinkCaps=%" GST_PTR_FORMAT "\n",
					(int)cdmidecryptor->mediaType, gst_buffer_get_size(buffer), ivSize, subSampleCount, cdmidecryptor->sinkCaps);
	errorCode = cdmidecryptor->drmSession->decrypt(keyIDBuffer, ivBuffer, buffer, subSampleCount, subsamplesBuffer, cdmidecryptor->sinkCaps);
	cdmidecryptor->drmSession->ReleaseAfterUse();
