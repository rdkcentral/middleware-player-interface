...
static void gst_widevinedecryptor_class_init(GstwidevinedecryptorClass * klass)
{
        ...
        // log the resolved src caps string so tune-time logs show exactly what secure-memory feature (if any) was advertised
        GST_INFO("widevinedecryptor: dynamic src caps string: %s (platformMemFeature=%s)", srcCapsStr, platformMemFeature ? platformMemFeature : "(none)");
        g_free(srcCapsStr);
        ...
        {
                g_print("surya - widevinedecryptor: failed to create dynamic src pad template caps\n");
                GST_ERROR("widevinedecryptor: failed to create dynamic src pad template caps");
        }
