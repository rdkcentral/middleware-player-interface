...
{
	g_print("patch 0005 applied: surya - on_decryptor_src_pad_event\n");
	...
	if (!srcpad)
	{
		...
		g_print("patch 0005 applied: surya - failed to retrieve src pad from decryptor %s\n", GST_ELEMENT_NAME(decryptor));
	...
	if (!parent)
	{
		...
		g_print("patch 0005 applied: surya - failed to get parent bin of decryptor %s\n", GST_ELEMENT_NAME(decryptor));
		gst_object_unref(srcpad);
	...
	if (!svppay)
	{
		...
		g_print("patch 0005 applied: surya - failed to create svppay element\n");
		gst_object_unref(srcpad);
		gst_object_unref(peer);
		return GST_PAD_PROBE_REMOVE;
	}
	g_print("patch 0005 applied: surya - successfully created svppay element\n");
	...
	else
	{
		...
		g_print("patch 0005 applied: surya - svppay inserted after decryptor %s\n", GST_ELEMENT_NAME(decryptor));
	}
...
static void element_setup_cb_svppay(GstElement *playbin, GstElement *element, gpointer user_data)
{
	...
	if (name && strstr(name, "decryptor"))
	{
		...
		if (!srcpad)
		{
			...
			g_print("patch 0005 surya: unable to retrieve src pad from element %s\n", GST_ELEMENT_NAME(element));
			return;
		}
