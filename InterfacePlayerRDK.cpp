...
void InterfacePlayerRDK::TearDownStream(int type)
{
	...
	// Acquire mMutex to safely access gstPrivateContext members
	// This ensures thread-safety even if called from other APIs in the future
	std::lock_guard<std::recursive_mutex> globalLock(mMutex);
...
bool InterfacePlayerRDK::WaitForSourceSetup(int mediaType)
{
	...
	while (timeRemaining >= 0)
	{
		...
		(void)status; // Condition variable status is checked implicitly
		if (mPauseInjector)
		{
