...
namespace
{
    ...
    uint32_t getValue(const gsw_CcFontStyle& attrib)
    {
        ...
        {
            MW_LOG_WARN("Cannot match %s to attribute value", attrib);
    ...
    uint32_t getValue(gsw_CcAttributes * attrib, short type)
    {
        switch(type)
        {
            ...
            default:
                MW_LOG_WARN("wrong attribute type used 0x%x",type);
                return -1;
    ...
    void setValue(gsw_CcAttributes* currentAttributes, gsw_CcAttributes * attrib, short type)
    {
        switch(type)
        {
            ...
            case GSW_CC_ATTRIB_EDGE_COLOR:
                ...
                return;
            default:
                MW_LOG_WARN("%s: wrong attribute type used 0x%x",__func__,  type);
                return;
