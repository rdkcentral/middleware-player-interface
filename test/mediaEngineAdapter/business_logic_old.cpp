 ...
 * @note This is typically the FIRST API to call
 */
std::string BusinessLogic::LoadManifest(std::string url)
{
    std::cout << "[INFO] LoadManifest called: " << url << std::endl;
    ...
    try {
        ...
        std::cout << "[INFO] Calling ConfigurePipeline..." << std::endl;
        ConfigurePipeline();
        std::cout << "[INFO] ConfigurePipeline completed successfully" << std::endl;
        ...
        std::cout << "[ERROR] ConfigurePipeline failed with unknown exception" << std::endl;
        currentState = PlayerState::ERROR;
        return "Error: Failed to load manifest - unknown error";
    }
}

/**
 * @brief Start or resume playback
 * @note Call LoadManifest BEFORE calling this API
 */
 ...
 * @note Can only pause when in PLAYING state
 */
 ...
 * @note Can only seek when in PLAYING or PAUSED state
 */
