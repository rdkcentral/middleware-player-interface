...
class PlaybackMPD(testController):
    ...
    def __init__(self, TestName=""):
        super().__init__(TestName, maxRunTime=5)
    ...
    def runTest(self):
        ...
        self.testCommandList = [
            ...
            #Pause playback 
            {"cmd": "pause"},
            {"expect": r"Pause executed. Result: true"},
            {"cmd": "getpositionmilliseconds"},
            {"expect" : r"GetPositionMilliseconds: ([1-9](\d{3}))"},
            {"expect" : r"GetPositionMilliseconds: ([1-9](\d{3}))"},
            {"expect" : r"GetPositionMilliseconds: ([1-9](\d{3}))"},
            {"cmd" : pi_cli, "postdelay": 2000},
            {"expect" : r"GetPositionMilliseconds: ([1-9](\d{3}))"},
            {"expect" : r"GetPositionMilliseconds: ([1-9](\d{3}))"},
          

             # # Resume playback - fix the expectation patterns
            {"cmd": "resume"},
            {"expect": r"Resume executed.*Success.*"},
            {"expect": r"Pipeline state: Playing"},
            {"cmd": "getpositionmilliseconds"},
            {"expect" : r"GetPositionMilliseconds: ([1-9](\d{3}))"},
            {"expect" : r"GetPositionMilliseconds: ([1-9](\d{3}))"},
            {"expect" : r"GetPositionMilliseconds: ([1-9](\d{3}))"},
