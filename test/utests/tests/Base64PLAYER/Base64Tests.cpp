...
TEST(Base64DecodeTests, ValidBase64StringDecoding) {
    ...
    unsigned char* decodedResult = base64_Decode(src, &decodedLen, strlen(src));
...
TEST(Base64DecodeTests, NullSourcePointer) {
    ...
    unsigned char* decodedResult = base64_Decode(NULL, &decodedLen, 0);
    std::cout << "base64_Decode returned pointer: " << static_cast<void*>(decodedResult) << " with decodedLen: " << decodedLen << std::endl;
...
TEST(Base64DecodeTests, NullLenPointerWithValidInput) {
    ...
    unsigned char* decodedResult = base64_Decode(src, NULL, strlen(src));
...
TEST(Base64DecodeTests, ValidBase64StringWithIncorrectPadding) {
    ...
    ASSERT_NE(decodedResult, nullptr);
    EXPECT_EQ(decodedLen, 5);
    
	//"Hello" → encoded = "SGVsbG8=" (with one = padding).
	//Without the = padding → "SGVsbG8"

	EXPECT_EQ(std::string(reinterpret_cast<char*>(decodedResult), decodedLen), "Hello");
