...
TEST_F(PlayerCCManagerInstantiableTest, EnablingTrickplayStatus) {
    ...
    EXPECT_NO_THROW(testPlayerCCManager->SetTrickplayStatus(true));
...
TEST_F(PlayerCCManagerInstantiableTest, DisablingTrickplayStatus) {
    ...
    EXPECT_NO_THROW(testPlayerCCManager->SetTrickplayStatus(false));
...
TEST_F(PlayerCCManagerInstantiableTest, SetStatus_EnableTrue_Positive) {
    ...
    int ret = testPlayerCCManager->SetStatus(true);
...
TEST_F(PlayerCCManagerInstantiableTest, SetStatus_EnableFalse_Positive) {
    ...
    int ret = testPlayerCCManager->SetStatus(false);
    std::cout << "Method returned: " << ret << std::endl;
...
TEST_F(PlayerCCManagerInstantiableTest, ValidCCTrackDefaultFormat) {
    ...
    int retStatus = testPlayerCCManager->SetTrack(track, eCLOSEDCAPTION_FORMAT_DEFAULT);
...
TEST_F(PlayerCCManagerInstantiableTest, ValidCCTrackAllEnumFormats) {
    ...
    for (size_t i = 0; i < sizeof(enumFormats)/sizeof(enumFormats[0]); i++) {
        ...
        int retStatus = testPlayerCCManager->SetTrack(track, enumFormats[i]);
...
TEST_F(PlayerCCManagerInstantiableTest, InvalidCCTrackEmpty) {
    ...
    for (auto format : enumFormats) {
        ...
        int retStatus = testPlayerCCManager->SetTrack(track, format);
...
TEST_F(PlayerCCManagerInstantiableTest, VerifyIsOOBCCRenderingSupportedReturnsTrue) {
    ...
    bool result = testPlayerCCManager->IsOOBCCRenderingSupported();
...
TEST_F(PlayerCCManagerInstantiableTest, SetParentalControlStatusEnabled) {
    ...
    EXPECT_NO_THROW({
        testPlayerCCManager->SetParentalControlStatus(true);
...
TEST_F(PlayerCCManagerInstantiableTest, SetParentalControlStatusDisabled) {
    ...
    EXPECT_NO_THROW({
        testPlayerCCManager->SetParentalControlStatus(false);
...
TEST_F(PlayerCCManagerInstantiableTest, BasicInitialization) {
    ...
    testPlayerCCManager->EnsureInitialized();
...
TEST_F(PlayerCCManagerInstantiableTest, UpdateSingleValidTrack) {
    ...
    testPlayerCCManager->updateLastTextTracks(tracks);
...
TEST_F(PlayerCCManagerInstantiableTest, ConfirmConsistency_AcrossMultipleInstances) {
    ...
    bool result1 = testPlayerCCManager->CheckCCHandle();
    std::cout << "instance1.CheckCCHandle() returned: " << result1 << std::endl;
    
    std::cout << "Invoking CheckCCHandle() on instance2" << std::endl;
    bool result2 = testPlayerCCManager->CheckCCHandle();
    std::cout << "instance2.CheckCCHandle() returned: " << result2 << std::endl;
    
    EXPECT_TRUE(result1);
    EXPECT_TRUE(result2);
#else
    GTEST_SKIP() << "Skipping ConfirmConsistency_AcrossMultipleInstances test due to L1TEST_SKIP being defined.";   
#endif
 ...
 * | 03               | Invoke testPlayerCCManager->EnsureHALInitialized() method                                  | player = instance, EnsureHALInitialized() = invoked | EnsureHALInitialized() completes without throwing errors                  | Should Pass     |
...
TEST_F(PlayerCCManagerInstantiableTest, EnsureHALInitializedCompletesWithoutErrorsOnFirstCall) {
    ...
    testPlayerCCManager->EnsureHALInitialized();
...
TEST_F(PlayerCCManagerInstantiableTest, EnsureRendererCommsInitialized_start) {
    ...
    EXPECT_NO_THROW({
        testPlayerCCManager->EnsureRendererCommsInitialized();
...
TEST_F(PlayerCCManagerInstantiableTest, ValidStyleSimple) {
    ...
    testPlayerCCManager->SetStatus(true);
    int ret = testPlayerCCManager->SetStyle(options);
 ...
 * | Variation / Step | Description                                          | Test Data                                        | Expected Result                                                                       | Notes         |
 * | :--------------: | ---------------------------------------------------- | ------------------------------------------------ | ------------------------------------------------------------------------------------- | ------------- |
 * | 01               | Invoke the first Stop() call on the player object    | function call: testPlayerCCManager->Stop()                     | Stop() executes without throwing exceptions and completes successfully               | Should Pass   |
 * | 02               | Invoke the second Stop() call to test idempotency      | function call: testPlayerCCManager->Stop()                     | Second Stop() call executes without throwing exceptions, confirming idempotency         | Should Pass   |
 */
...
TEST_F(PlayerCCManagerInstantiableTest, InitializeWithValidPointer) {
    ...
    int retVal = testPlayerCCManager->Init(validHandle);
...
TEST_F(PlayerCCManagerInstantiableTest, InitializeWithNullPointer) {
    ...
    int retVal = testPlayerCCManager->Init(nullHandle);
    std::cout << "Public Init() returned value: " << retVal << std::endl;
