...
TEST(PlayerIsoBmffBuffer, DefaultConstructorInitializesAllMembers) {
    ...
    EXPECT_NO_THROW({
        ...
        std::cout << "boxes vector size: " << obj.boxes.size() << std::endl;
        EXPECT_EQ(obj.boxes.size(), 0);

        // Debug log: Print buffer pointer value (expected to be NULL)
        std::cout << "buffer: " << static_cast<const void*>(obj.buffer) << std::endl;
        EXPECT_EQ(obj.buffer, nullptr);

        // Debug log: Print bufSize value (expected to be 0)
        std::cout << "bufSize: " << obj.bufSize << std::endl;
        EXPECT_EQ(obj.bufSize, 0);

        // Debug log: Print chunkedBox pointer value (expected to be NULL)
        std::cout << "chunkedBox: " << obj.chunkedBox << std::endl;
        EXPECT_EQ(obj.chunkedBox, nullptr);

        // Debug log: Print mdatCount value (expected to be 0)
        std::cout << "mdatCount: " << obj.mdatCount << std::endl;
        EXPECT_EQ(obj.mdatCount, 0);
    });
