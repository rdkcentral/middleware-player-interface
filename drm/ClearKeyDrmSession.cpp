 ...
 */
void ClearKeySession::initDRMSession()
{
#if OPENSSL_VERSION_NUMBER >= 0x10100000L
...
#endif
	MW_LOG_ERR("ClearKeySession: enter ");
}
...
static unsigned char * extractKeyIdFromPssh(const char* psshData, int dataLength, int *len)
{
	...
	*len = (int)16;
	MW_LOG_INFO("ck keyid: %s keyIdlen: %d", key_id, 16);
	if (PlayerLogManager::isLogLevelAllowed(mLOGLEVEL_TRACE))
	{
		DumpBinaryBlob(key_id, 16);
	}
 ...
 */
void ClearKeySession::generateDRMSession(const uint8_t *f_pbInitData,
		uint32_t f_cbInitData, std::string &customData)
{
	...
	if(f_pbInitData != NULL && f_cbInitData > 0)
	{
		...
		if (keyId == NULL)
		{
			MW_LOG_ERR("ClearKeySession: ERROR: Key Id not found in initdata");
		...
		else
		{
			...
			{
				MW_LOG_ERR("ClearKeySession: ERROR: Invalid keyID length %d", keyIDLen);
				m_eKeyState = KEY_ERROR;
			}
			else
			{
				setKeyId(reinterpret_cast<const char*>(keyId), keyIDLen);
				MW_LOG_ERR("ClearKeySession: Session generated for clearkey");
			}
			free(keyId);
		}
	}
 ...
 */
DrmData * ClearKeySession::generateKeyRequest(string& destinationURL, uint32_t timeout)
{
	DrmData *licenseChallenge = NULL;
	if(NULL == m_keyId || m_keyIdLen <= 0)
	{
		MW_LOG_ERR("ClearKeySession: Error generating license request ");
	}
	else
	{
		char* urlEncodedkeyId = base64_URL_Encode(m_keyId, m_keyIdLen);
		if(urlEncodedkeyId)
		{
 ...
 */
int ClearKeySession::processDRMKey(DrmData* key, uint32_t timeout)
{
	int ret = 0;
	if (!key)
	{
		MW_LOG_ERR("ClearKeySession: Cannot Process Null Key ");
		return ret;
	}

	std::string keyDataStr = key->getData();
	MW_LOG_INFO("ClearKeySession: Processing license response %s", keyDataStr.c_str());
	if (m_eKeyState == KEY_PENDING)
	{
		...
		if (licenseResponse)
		{
			if (cJSON_HasObjectItem(licenseResponse, "keys"))
			{
				...
				if (keyJsonObj && keyIdJsonObj)
				{
					...
					if (resKeyIdLen == m_keyIdLen && 0 == memcmp(m_keyId, resKeyId, m_keyIdLen))
					{
						...
						if (resKeyLen == AES_CTR_KEY_LEN)
						{
							...
							MW_LOG_INFO("ClearKeySession: Got key from license response keyLength %zu", m_keyLen);
							ret = 1;
						}
						else
						{
				...
				{
					MW_LOG_ERR("ClearKeySession: ERROR : Failed parse Key info, from response");
					m_eKeyState = KEY_ERROR;
				}
			}
		...
		{
			MW_LOG_ERR("ClearKeySession: ERROR : Failed parse response JSON");
			m_eKeyState = KEY_ERROR;
		}
	}
	else
	{
		MW_LOG_ERR("ClearKeySession: ERROR : Invalid DRM state : DRMState %d", m_eKeyState);
	}
	return ret;
}
                ...
                GstBuffer* subSamplesBuffer, GstCaps* caps)
{
	...
	else
	{
		...
		if (!bufferMapped)
		{
			MW_LOG_ERR("ClearKeySession: ERROR : Failed to map buffer");
		}

		ivMapped = gst_buffer_map(ivBuffer, &ivMap, GST_MAP_READ);
		if (!ivMapped)
		{
			MW_LOG_ERR("ClearKeySession: ERROR : Failed to map ivBuffer");
		}
		...
		if(subSampleCount > 0)
		{
			...
			if (!subSampleMapped)
			{
				MW_LOG_ERR("ClearKeySession: ERROR : Failed to map subSamplesBuffer");
			}
		}
	}
	...
	if(bufferMapped && ivMapped && (subSampleCount ==0 || subSampleMapped))
	{
		...
		{
			MW_LOG_ERR("ClearKeySession: ERROR : Failed to allocate subsample reader");
		}
	}
		...
		const uint8_t *payloadData, uint32_t payloadDataSize, uint8_t **ppOpaqueData=NULL)
{
	...
	if (m_eKeyState == KEY_READY)
	{
		...
		if(decryptedDataBuf)
		{
			...
			{
				MW_LOG_TRACE("ClearKeySession: invalid IV size %u", f_cbIV);
			}
		}
		...
		if (decryptedDataBuf && ivBuff)
		{
			...
			else
			{
				if (!EVP_DecryptUpdate(OPEN_SSL_CONTEXT,(unsigned char*) decryptedDataBuf, (int*) &decLen, (const unsigned char*) payloadData,
					payloadDataSize))
				{
					MW_LOG_TRACE("ClearKeySession: EVP_DecryptUpdate failed");
				}
				else
				{
					decryptedDataLen = decLen;
					decLen = 0;
					MW_LOG_TRACE("ClearKeySession: EVP_DecryptUpdate success decryptedDataLen = %d payload Data length = %d", (int) decryptedDataLen, (int)payloadDataSize);
					if (!EVP_DecryptFinal_ex(OPEN_SSL_CONTEXT, (unsigned char*) (decryptedDataBuf + decryptedDataLen), (int*) &decLen))
					{
						MW_LOG_TRACE("ClearKeySession: EVP_DecryptFinal_ex failed mDrmState = %d", (int) m_eKeyState);
					}
					else
					{
						decryptedDataLen += decLen;
						MW_LOG_TRACE("ClearKeySession: decrypt success");
						status = 0;
					}
				}
			}
	...
	{
		MW_LOG_ERR( "ClearKeySession: key not ready! mDrmState = %d", m_eKeyState);
	}
	return status;
}
