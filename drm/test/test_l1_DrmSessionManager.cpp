 ...
 */
TEST(DrmSessionContext, VerifyDefaultConstruction) {
 ...
 */
TEST(DrmSessionContext, CopyConstructorValidNonEmpty)
{
 ...
 */
TEST(DrmSessionContext, CopyConstructorValidEmptyNull)
{
 ...
 */
TEST(DrmSessionContext, DeepCopyDataVector)
{
 ...
 */
TEST(DrmSessionContext, DestructorExecutesWithoutExceptions) {
 ...
 */
TEST(DrmSessionManager, ConstructValidPositive) {
    ...
    DrmSessionManager drmSessionManager(maxDrmSessions, player);
 ...
 */
TEST(DrmSessionManager, ConstructBoundaryValue) {
    ...
    // Log input values
    std::cout << "Invoking DrmSessionManager constructor with boundary maxDrmSessions = " 
              << maxDrmSessions << " and player pointer = " << player << std::endl;
 ...
 */
TEST(DrmSessionManager, ConstructZeroSessions) {
    ...
    // Log input values
    std::cout << "Invoking DrmSessionManager constructor with maxDrmSessions = " 
              << maxDrmSessions << " (zero sessions) and player pointer = " << player << std::endl;
 ...
 */
TEST(DrmSessionManager, ConstructNegativeSessions) {
    ...
    // Log input values
    std::cout << "Invoking DrmSessionManager constructor with negative maxDrmSessions = " 
              << maxDrmSessions << " and player pointer = " << player << std::endl;
 ...
 */
TEST(DrmSessionManager, ConstructHighSessionValue) {
    ...
    // Log input values
    std::cout << "Invoking DrmSessionManager constructor with very high maxDrmSessions = " 
              << maxDrmSessions << " and player pointer = " << player << std::endl;
 ...
 */
TEST(DrmSessionManager, ConstructNullPlayer) {
    ...
    // Log input values
    std::cout << "Invoking DrmSessionManager constructor with maxDrmSessions = " 
              << maxDrmSessions << " and player pointer = " << player << " (nullptr)" << std::endl;
 ...
 */
TEST(DrmSessionManager, ProcessedKeyIdAlreadyCached) {
    ...
    DrmSessionManager drmSessionManager(5, nullptr);
 ...
 */
TEST(DrmSessionManager, EmptyKeyIdArray) {
 ...
 * | Variation / Step | Description | Test Data | Expected Result | Notes |
 * | :----: | --------- | ---------- |-------------- | ----- |
 * | 01 | Create a valid DrmHelper pointer and a DrmSessionManager instance using custom constructor | drmHelper = valid pointer, capacity = 10, dependency = nullptr | Objects are successfully created | Should be successful |
 ...
 * | 03 | Iterate through each media type and call QueueContentProtection to verify it returns true | drmHelper = valid pointer, periodId = "period_enum", adapIdx = 2, each GstMediaType from mediaTypes, isVssPeriod = true | Return value is true and EXPECT_TRUE(ret) assertion passes | Should Pass |
 */
TEST(DrmSessionManager, QueueContentProtection_ValidCP_VssTrue) 
{
    std::cout << "Entering QueueContentProtection_ValidCP_VssTrue test" << std::endl;

    // Create a valid DrmHelper pointer
    auto drmHelper = std::make_shared<DrmHelper>();
    // Create a DrmSessionManager object using a custom constructor
    DrmSessionManager drmSessionManager(10, nullptr);
    std::string periodId = "period_enum";
    uint32_t adapIdx = 2;

    // Prepare list of media types for testing, with isVssPeriod = true
    std::vector<GstMediaType> mediaTypes = {
        eGST_MEDIATYPE_VIDEO,
        eGST_MEDIATYPE_AUDIO,
        eGST_MEDIATYPE_SUBTITLE,
        eGST_MEDIATYPE_AUX_AUDIO,
        eGST_MEDIATYPE_MANIFEST,
        eGST_MEDIATYPE_LICENCE,
        eGST_MEDIATYPE_IFRAME,
        eGST_MEDIATYPE_INIT_VIDEO,
        eGST_MEDIATYPE_INIT_AUDIO,
        eGST_MEDIATYPE_INIT_SUBTITLE,
        eGST_MEDIATYPE_INIT_AUX_AUDIO,
        eGST_MEDIATYPE_PLAYLIST_VIDEO,
        eGST_MEDIATYPE_PLAYLIST_AUDIO,
        eGST_MEDIATYPE_PLAYLIST_SUBTITLE,
        eGST_MEDIATYPE_PLAYLIST_AUX_AUDIO,
        eGST_MEDIATYPE_PLAYLIST_IFRAME,
        eGST_MEDIATYPE_INIT_IFRAME,
        eGST_MEDIATYPE_DSM_CC,
        eGST_MEDIATYPE_IMAGE,
        eGST_MEDIATYPE_DEFAULT
    };

    for(auto type : mediaTypes)
    {
        std::cout << "Invoking QueueContentProtection with parameters:" << std::endl;
        std::cout << "  drmHelper: valid pointer" << std::endl;
        std::cout << "  periodId: " << periodId << std::endl;
        std::cout << "  adapIdx: " << adapIdx << std::endl;
        std::cout << "  GstMediaType: " << type << std::endl;
        std::cout << "  isVssPeriod: true" << std::endl;

        bool ret = drmSessionManager.QueueContentProtection(drmHelper, periodId, adapIdx, type, true);
        std::cout << "Method QueueContentProtection returned: " << ret << std::endl;
        EXPECT_TRUE(ret);
    }

    std::cout << "Exiting QueueContentProtection_ValidCP_VssTrue test" << std::endl;
}
/**
 * @brief Verify QueueContentProtection API returns true for valid content protection when isVssPeriod is false
 *
 * This test verifies that the QueueContentProtection API is successfully invoked with valid parameters. The test creates a valid DrmHelper pointer, initializes a DrmSessionManager object, sets up specific input parameters (periodId, adapIdx) and iterates over a list of valid GstMediaType values with isVssPeriod set to false. For each media type, the API is called and the returned boolean value is asserted to be true.
 *
 * **Test Group ID:** Basic: 01
 * **Test Case ID:** 017
 * **Priority:** High
 ...
 * | Variation / Step | Description                                                                                       | Test Data                                                                                                | Expected Result                                                      | Notes            |
 * | :--------------: | ------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------- | ---------------- |
 * | 01               | Instantiate a valid DrmHelper pointer and initialize DrmSessionManager with specified parameters   | drmHelper = valid pointer, capacity = 10, pointer = nullptr                                              | Objects created successfully                                         | Should be successful |
 ...
 * | 04               | For each media type, log parameters and invoke QueueContentProtection with isVssPeriod as false     | drmHelper, periodId, adapIdx, current GstMediaType, isVssPeriod = false                                    | API returns true for each invocation                                 | Should Pass      |
 * | 05               | Verify the return value of QueueContentProtection using EXPECT_TRUE assertion                      | ret = return value from QueueContentProtection                                                             | Boolean value true confirming correct API behavior                   | Should Pass      |
 */
TEST(DrmSessionManager, QueueContentProtection_ValidCP_VssFalse) 
{
    std::cout << "Entering QueueContentProtection_ValidCP_VssFalse test" << std::endl;
    ...
    // Prepare list of media types for testing, with isVssPeriod = false
    std::vector<GstMediaType> mediaTypes = {
        eGST_MEDIATYPE_VIDEO,
        eGST_MEDIATYPE_AUDIO,
        eGST_MEDIATYPE_SUBTITLE,
    ...
    for(auto type : mediaTypes)
    {
        ...
        std::cout << "  adapIdx: " << adapIdx << std::endl;
        std::cout << "  GstMediaType: " << type << std::endl;
        std::cout << "  isVssPeriod: false" << std::endl;

        bool ret = drmSessionManager.QueueContentProtection(drmHelper, periodId, adapIdx, type, false);
        std::cout << "Method QueueContentProtection returned: " << ret << std::endl;
        EXPECT_TRUE(ret);
    }

    std::cout << "Exiting QueueContentProtection_ValidCP_VssFalse test" << std::endl;
}
/**
 * @brief Validate that QueueContentProtection returns false when provided with a null DrmHelper.
 *
 * This test verifies that the QueueContentProtection method of DrmSessionManager handles the case where the DrmHelper pointer is null correctly.
 *
 * **Test Group ID:** Basic: 01@n
 * **Test Case ID:** 018@n
 * **Priority:** High@n
 ...
 * | Variation / Step | Description                                                                                           | Test Data                                                                                                  | Expected Result                                                                           | Notes         |
 * | :--------------: | ----------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------- | ------------- |
 * | 01               | Create an instance of DrmSessionManager with a capacity of 10 and a null DrmHelper.                   | capacity = 10, drmHelper = nullptr                                                                           | Instance created successfully.                                                            | Should be successful |
 ...
 * | 03               | Invoke QueueContentProtection with a null DrmHelper and the initialized parameters.                 | drmHelper = nullptr, periodId = period1, adapIdx = 0, GstMediaType = eGST_MEDIATYPE_VIDEO, isVssPeriod = false   | Function returns false; assertion (EXPECT_FALSE(ret)) passes confirming correct behavior.  | Should Pass   |
 * | 04               | Log the method execution and exit information to trace execution.                                  | N/A                                                                                                        | Log messages printed as expected.                                                         | Should be successful |
 */
TEST(DrmSessionManager, QueueContentProtection_NullDrmHelper) 
{
    std::cout << "Entering QueueContentProtection_NullDrmHelper test" << std::endl;

    // Create a DrmSessionManager object using a custom constructor
    DrmSessionManager drmSessionManager(10, nullptr);
    std::string periodId = "period1";
    uint32_t adapIdx = 0;
    GstMediaType type = eGST_MEDIATYPE_VIDEO;
    // drmHelper is nullptr
    std::shared_ptr<DrmHelper> drmHelper = nullptr;

    std::cout << "Invoking QueueContentProtection with parameters:" << std::endl;
    std::cout << "  drmHelper: nullptr" << std::endl;
    std::cout << "  periodId: " << periodId << std::endl;
    std::cout << "  adapIdx: " << adapIdx << std::endl;
    std::cout << "  GstMediaType: " << type << std::endl;
    std::cout << "  isVssPeriod: false" << std::endl;
    ...
    std::cout << "Exiting QueueContentProtection_NullDrmHelper test" << std::endl;
}
/**
 * @brief Verify that QueueContentProtection returns false when provided with an empty periodId
 *
 * This test verifies that when an empty periodId is passed to the QueueContentProtection API of DrmSessionManager, 
 * the function returns false. It ensures that the API correctly handles invalid period identifiers.
 *
 * **Test Group ID:** Basic: 01@n
 * **Test Case ID:** 019@n
 * **Priority:** High@n
 ...
 * | Variation / Step | Description                                                  | Test Data                                                                                                       | Expected Result                                             | Notes            |
 * | :--------------: | ------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------- | ---------------- |
 * | 01               | Set up valid DrmHelper pointer and instantiate DrmSessionManager | drmHelper = valid pointer, drmSessionManager = (10, nullptr)                                                    | Objects are successfully created                           | Should be successful |
 ...
 * | 03               | Invoke QueueContentProtection API with an empty periodId       | drmHelper = valid pointer, periodId = (empty string), adapIdx = 0, GstMediaType = eGST_MEDIATYPE_VIDEO, isVssPeriod = false | API returns false and EXPECT_FALSE assertion passes        | Should Fail      |
 */
TEST(DrmSessionManager, QueueContentProtection_EmptyPeriodId) 
{
    std::cout << "Entering QueueContentProtection_EmptyPeriodId test" << std::endl;

    // Create a valid DrmHelper pointer
    auto drmHelper = std::make_shared<DrmHelper>();
    // Create a DrmSessionManager object using a custom constructor
    DrmSessionManager drmSessionManager(10, nullptr);
    // Using an empty periodId string
    std::string periodId = "";
    uint32_t adapIdx = 0;
    GstMediaType type = eGST_MEDIATYPE_VIDEO;
    ...
    std::cout << "Invoking QueueContentProtection with parameters:" << std::endl;
    std::cout << "  drmHelper: valid pointer" << std::endl;
    std::cout << "  periodId: (empty string)" << std::endl;
    std::cout << "  adapIdx: " << adapIdx << std::endl;
    std::cout << "  GstMediaType: " << type << std::endl;
    std::cout << "  isVssPeriod: false" << std::endl;
    ...
    std::cout << "Exiting QueueContentProtection_EmptyPeriodId test" << std::endl;
}
/**
 * @brief Test function to verify that QueueContentProtection returns false for an excessively large adaptation index.
 *
 * This test verifies that when QueueContentProtection is invoked with an excessively large adaptation index,
 * the function returns false as expected, indicating that the input is out of the allowed range.
 * It ensures robustness by checking the boundary conditions related to content protection queuing.
 *
 * **Test Group ID:** Basic: 01
 * **Test Case ID:** 020
 * **Priority:** High
...
TEST(DrmSessionManager, QueueContentProtection_ExcessiveAdapIdx) 
{
    ...
    DrmSessionManager drmSessionManager(10, nullptr);
    std::string periodId = "period1";
    // Excessively large adaptation index
    uint32_t adapIdx = 999999;
    GstMediaType type = eGST_MEDIATYPE_VIDEO;
    ...
    std::cout << "Exiting QueueContentProtection_ExcessiveAdapIdx test" << std::endl;
}
/**
 * @brief Validate that QueueProtectionEvent accepts valid inputs for all media types
 *
 ...
 * | Variation / Step | Description | Test Data | Expected Result | Notes |
 * | :----: | --------- | ---------- | -------------- | ----- |
 * | 01 | Invoke QueueProtectionEvent with eGST_MEDIATYPE_VIDEO | drmHelper = valid, periodId = period_all, adapIdx = 2, type = eGST_MEDIATYPE_VIDEO | Protection event queued successfully | Should Pass |
 * | 02 | Invoke QueueProtectionEvent with eGST_MEDIATYPE_AUDIO | drmHelper = valid, periodId = period_all, adapIdx = 2, type = eGST_MEDIATYPE_AUDIO | Protection event queued successfully | Should Pass |
 * | 03 | Invoke QueueProtectionEvent with eGST_MEDIATYPE_SUBTITLE | drmHelper = valid, periodId = period_all, adapIdx = 2, type = eGST_MEDIATYPE_SUBTITLE | Protection event queued successfully | Should Pass |
 * | 04 | Invoke QueueProtectionEvent with eGST_MEDIATYPE_AUX_AUDIO | drmHelper = valid, periodId = period_all, adapIdx = 2, type = eGST_MEDIATYPE_AUX_AUDIO | Protection event queued successfully | Should Pass |
 * | 05 | Invoke QueueProtectionEvent with eGST_MEDIATYPE_MANIFEST | drmHelper = valid, periodId = period_all, adapIdx = 2, type = eGST_MEDIATYPE_MANIFEST | Protection event queued successfully | Should Pass |
 * | 06 | Invoke QueueProtectionEvent with eGST_MEDIATYPE_LICENCE | drmHelper = valid, periodId = period_all, adapIdx = 2, type = eGST_MEDIATYPE_LICENCE | Protection event queued successfully | Should Pass |
 * | 07 | Invoke QueueProtectionEvent with eGST_MEDIATYPE_IFRAME | drmHelper = valid, periodId = period_all, adapIdx = 2, type = eGST_MEDIATYPE_IFRAME | Protection event queued successfully | Should Pass |
 * | 08 | Invoke QueueProtectionEvent with eGST_MEDIATYPE_INIT_VIDEO | drmHelper = valid, periodId = period_all, adapIdx = 2, type = eGST_MEDIATYPE_INIT_VIDEO | Protection event queued successfully | Should Pass |
 * | 09 | Invoke QueueProtectionEvent with eGST_MEDIATYPE_INIT_AUDIO | drmHelper = valid, periodId = period_all, adapIdx = 2, type = eGST_MEDIATYPE_INIT_AUDIO | Protection event queued successfully | Should Pass |
 * | 10 | Invoke QueueProtectionEvent with eGST_MEDIATYPE_INIT_SUBTITLE | drmHelper = valid, periodId = period_all, adapIdx = 2, type = eGST_MEDIATYPE_INIT_SUBTITLE | Protection event queued successfully | Should Pass |
 * | 11 | Invoke QueueProtectionEvent with eGST_MEDIATYPE_INIT_AUX_AUDIO | drmHelper = valid, periodId = period_all, adapIdx = 2, type = eGST_MEDIATYPE_INIT_AUX_AUDIO | Protection event queued successfully | Should Pass |
 * | 12 | Invoke QueueProtectionEvent with eGST_MEDIATYPE_PLAYLIST_VIDEO | drmHelper = valid, periodId = period_all, adapIdx = 2, type = eGST_MEDIATYPE_PLAYLIST_VIDEO | Protection event queued successfully | Should Pass |
 * | 13 | Invoke QueueProtectionEvent with eGST_MEDIATYPE_PLAYLIST_AUDIO | drmHelper = valid, periodId = period_all, adapIdx = 2, type = eGST_MEDIATYPE_PLAYLIST_AUDIO | Protection event queued successfully | Should Pass |
 * | 14 | Invoke QueueProtectionEvent with eGST_MEDIATYPE_PLAYLIST_SUBTITLE | drmHelper = valid, periodId = period_all, adapIdx = 2, type = eGST_MEDIATYPE_PLAYLIST_SUBTITLE | Protection event queued successfully | Should Pass |
 * | 15 | Invoke QueueProtectionEvent with eGST_MEDIATYPE_PLAYLIST_AUX_AUDIO | drmHelper = valid, periodId = period_all, adapIdx = 2, type = eGST_MEDIATYPE_PLAYLIST_AUX_AUDIO | Protection event queued successfully | Should Pass |
 * | 16 | Invoke QueueProtectionEvent with eGST_MEDIATYPE_PLAYLIST_IFRAME | drmHelper = valid, periodId = period_all, adapIdx = 2, type = eGST_MEDIATYPE_PLAYLIST_IFRAME | Protection event queued successfully | Should Pass |
 * | 17 | Invoke QueueProtectionEvent with eGST_MEDIATYPE_INIT_IFRAME | drmHelper = valid, periodId = period_all, adapIdx = 2, type = eGST_MEDIATYPE_INIT_IFRAME | Protection event queued successfully | Should Pass |
 * | 18 | Invoke QueueProtectionEvent with eGST_MEDIATYPE_DSM_CC | drmHelper = valid, periodId = period_all, adapIdx = 2, type = eGST_MEDIATYPE_DSM_CC | Protection event queued successfully | Should Pass |
 * | 19 | Invoke QueueProtectionEvent with eGST_MEDIATYPE_IMAGE | drmHelper = valid, periodId = period_all, adapIdx = 2, type = eGST_MEDIATYPE_IMAGE | Protection event queued successfully | Should Pass |
 * | 20 | Invoke QueueProtectionEvent with eGST_MEDIATYPE_DEFAULT | drmHelper = valid, periodId = period_all, adapIdx = 2, type = eGST_MEDIATYPE_DEFAULT | Protection event queued successfully | Should Pass |
 */
TEST(DrmSessionManager, QueueProtectionEvent_validInputLoopAllMediaTypes) {
    std::cout << "Entering QueueProtectionEvent_validInputLoopAllMediaTypes test" << std::endl;
    
    // Create a valid DrmHelper pointer.
    auto drmHelper = std::make_shared<DrmHelper>();
    std::string periodId = "period_all";
    uint32_t adapIdx = 2;

    // Define an array of all GstMediaType values to test.
    GstMediaType mediaTypes[] = {
        eGST_MEDIATYPE_VIDEO,
        eGST_MEDIATYPE_AUDIO,
        eGST_MEDIATYPE_SUBTITLE,
        eGST_MEDIATYPE_AUX_AUDIO,
    ...
    // Instantiate DrmSessionManager using its constructor.
    DrmSessionManager drmSessionManager(5, nullptr);

    // Loop through each media type and invoke QueueProtectionEvent.
    for (auto type : mediaTypes) {
        std::cout << "Invoking QueueProtectionEvent with parameters:" << std::endl;
        std::cout << "  drmHelper: valid" << std::endl;
        std::cout << "  periodId: " << periodId << std::endl;
        std::cout << "  adapIdx: " << adapIdx << std::endl;
        std::cout << "  type: " << static_cast<int>(type) << std::endl;
        drmSessionManager.QueueProtectionEvent(drmHelper, periodId, adapIdx, type);
        std::cout << "Protection event queued for type: " << static_cast<int>(type) << std::endl;
    }

    std::cout << "Exiting QueueProtectionEvent_validInputLoopAllMediaTypes test" << std::endl;
}
/**
 * @brief Test that QueueProtectionEvent handles a null DrmHelper input without throwing exceptions.
 *
 * This test verifies that when a null DrmHelper pointer is passed to the QueueProtectionEvent method along with valid parameters, the function gracefully handles the null input without throwing any exceptions.
 *
 * **Test Group ID:** Basic: 01@n
 * **Test Case ID:** 022@n
 * **Priority:** High@n
 ...
 * | Variation / Step | Description | Test Data | Expected Result | Notes |
 * | :----: | --------- | ---------- |-------------- | ----- |
 * | 01 | Instantiate a null DrmHelper pointer and set valid test parameters (periodId, adapIdx, type) | drmHelper = nullptr, periodId = "period1", adapIdx = 1, type = eGST_MEDIATYPE_VIDEO | DrmSessionManager is instantiated successfully and inputs are set as intended | Should be successful |
 * | 02 | Invoke QueueProtectionEvent with the null DrmHelper and verify that no exception is thrown | Input: drmHelper = nullptr, periodId = "period1", adapIdx = 1, type = eGST_MEDIATYPE_VIDEO; Output: No exception thrown | The function call should not throw an exception | Should Pass |
 */
TEST(DrmSessionManager, QueueProtectionEvent_nullDrmHelper) {
    std::cout << "Entering QueueProtectionEvent_nullDrmHelper test" << std::endl;
    
    // Create a null DrmHelper pointer.
    std::shared_ptr<DrmHelper> drmHelper(nullptr);
    std::string periodId = "period1";
    uint32_t adapIdx = 1;
    GstMediaType type = eGST_MEDIATYPE_VIDEO;
    
    // Instantiate DrmSessionManager.
    DrmSessionManager drmSessionManager(5, nullptr);
    ...
    std::cout << "  adapIdx: " << adapIdx << std::endl;
    std::cout << "  type: " << static_cast<int>(type) << std::endl;
    EXPECT_NO_THROW(drmSessionManager.QueueProtectionEvent(drmHelper, periodId, adapIdx, type));
    std::cout << "Handled null drmHelper input gracefully without crashing" << std::endl;

    std::cout << "Exiting QueueProtectionEvent_nullDrmHelper test" << std::endl;
}
/**
 * @brief Test for QueueProtectionEvent with an empty periodId
 *
 * This test verifies that the DrmSessionManager handles an empty periodId gracefully without throwing exceptions.
 *
 * **Test Group ID:** Basic: 01@n
 * **Test Case ID:** 023@n
 * **Priority:** High@n
 ...
 * | Variation / Step | Description                                             | Test Data                                                                                  | Expected Result                                                  | Notes         |
 * | :----:           | ------------------------------------------------------- | ------------------------------------------------------------------------------------------ | ---------------------------------------------------------------- | ------------- |
 * | 01               | Set up the test environment by creating a valid DrmHelper, initializing an empty periodId, and assigning test values for adapIdx and type. | drmHelper = valid, periodId = "", adapIdx = 1, type = eGST_MEDIATYPE_AUDIO                  | All test parameters are initialized correctly.                   | Should be successful |
 * | 02               | Invoke the QueueProtectionEvent method with the initialized parameters. | drmHelper = valid, periodId = "", adapIdx = 1, type = eGST_MEDIATYPE_AUDIO                  | The method call does not throw any exceptions (EXPECT_NO_THROW passes). | Should Pass    |
 */
TEST(DrmSessionManager, QueueProtectionEvent_emptyPeriodId) {
    std::cout << "Entering QueueProtectionEvent_emptyPeriodId test" << std::endl;
    
    // Create a valid DrmHelper pointer.
    auto drmHelper = std::make_shared<DrmHelper>();
    std::string periodId = "";
    uint32_t adapIdx = 1;
    GstMediaType type = eGST_MEDIATYPE_AUDIO;
    ...
    std::cout << "  type: " << static_cast<int>(type) << std::endl;
    EXPECT_NO_THROW(drmSessionManager.QueueProtectionEvent(drmHelper, periodId, adapIdx, type));
    std::cout << "Processed empty periodId input appropriately without crashing" << std::endl;

    std::cout << "Exiting QueueProtectionEvent_emptyPeriodId test" << std::endl;
}
/**
 * @brief Verify that QueueProtectionEvent handles an extreme adaptation index value without crashing
 *
 * This test case ensures that calling QueueProtectionEvent on a valid DrmSessionManager instance with an extremely high adapIdx value (UINT32_MAX) is handled correctly. It verifies that no exception is thrown when processing edge case values, ensuring the robustness of the API for potential stress scenarios.
 *
 * **Test Group ID:** Basic: 01@n
 * **Test Case ID:** 024@n
 * **Priority:** High@n
 ...
 *
 * **Test Procedure:**
 * | Variation / Step | Description                                                         | Test Data: drmHelper = valid, periodId = period2, adapIdx = UINT32_MAX, type = eGST_MEDIATYPE_SUBTITLE | Expected Result: No exceptions thrown and proper handling of extreme adapIdx            | Notes       |
 * | :--------------: | ------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------------------------- | ----------- |
 * | 01               | Invoke QueueProtectionEvent API using an extreme adapIdx value        | drmHelper = valid, periodId = period2, adapIdx = UINT32_MAX, type = eGST_MEDIATYPE_SUBTITLE                  | API call should complete without throwing exceptions, indicating robust handling of extreme input | Should Pass |
 */
TEST(DrmSessionManager, QueueProtectionEvent_extremeAdapIdx) {
    std::cout << "Entering QueueProtectionEvent_extremeAdapIdx test" << std::endl;
    
    // Create a valid DrmHelper pointer.
    auto drmHelper = std::make_shared<DrmHelper>();
    std::string periodId = "period2";
    uint32_t adapIdx = UINT32_MAX;
    GstMediaType type = eGST_MEDIATYPE_SUBTITLE;
    ...
    std::cout << "  drmHelper: valid" << std::endl;
    std::cout << "  periodId: " << periodId << std::endl;
    std::cout << "  adapIdx: " << adapIdx << " (extreme value)" << std::endl;
    std::cout << "  type: " << static_cast<int>(type) << std::endl;
    EXPECT_NO_THROW(drmSessionManager.QueueProtectionEvent(drmHelper, periodId, adapIdx, type));
    std::cout << "Processed extreme adapIdx input appropriately without crashing" << std::endl;

    std::cout << "Exiting QueueProtectionEvent_extremeAdapIdx test" << std::endl;
}
/**
 ...
 */
TEST(DrmSessionManager, RegisterValidNonEmptyLambdaCallback)
{
    std::cout << "Entering RegisterValidNonEmptyLambdaCallback test" << std::endl;
    DrmSessionManager manager(5, nullptr);
    ...
    manager.RegisterHandleContentProtectionCb(validLambda);
    std::cout << "Callback registration complete. Checking if callback is set: " 
              << (manager.ContentUpdateCb ? "Yes" : "No") << std::endl;
    EXPECT_TRUE(static_cast<bool>(manager.ContentUpdateCb));
    
    std::cout << "Optionally invoking registered callback" << std::endl;
    std::vector<uint8_t> keyId = {1, 2, 3};
    std::string result = manager.ContentUpdateCb(nullptr, 1, keyId, 10);
 ...
 */
TEST(DrmSessionManager, RegisterEmptyCallback)
{
    std::cout << "Entering RegisterEmptyCallback test" << std::endl;
    DrmSessionManager manager(5, nullptr);
    std::cout << "Created DrmSessionManager with maxDrmSessions=5, player=nullptr" << std::endl;
    
    ContentUpdateCallback emptyCallback; // Default constructed callback
    std::cout << "Registering empty default callback" << std::endl;
    manager.RegisterHandleContentProtectionCb(emptyCallback);
    std::cout << "Callback registration complete. Checking if callback is empty: " 
              << (manager.ContentUpdateCb ? "Yes" : "No") << std::endl;
    EXPECT_FALSE(static_cast<bool>(manager.ContentUpdateCb));
 ...
 */
TEST(DrmSessionManager, RegisterLambdaReturningEmptyString)
{
    ...
    manager.RegisterHandleContentProtectionCb(emptyStringLambda);
    std::cout << "Callback registration complete. Checking if callback is set: " 
              << (manager.ContentUpdateCb ? "Yes" : "No") << std::endl;
    EXPECT_TRUE(static_cast<bool>(manager.ContentUpdateCb));
    ...
    std::string result = manager.ContentUpdateCb(nullptr, 2, keyId, 20);
    std::cout << "Callback invocation returned: '" << result << "'" << std::endl;
    EXPECT_EQ(result, "");
 ...
 */
TEST(DrmSessionManager, RegisterLambdaThrowingException)
{
    ...
    manager.RegisterHandleContentProtectionCb(throwingLambda);
    std::cout << "Callback registration complete. Checking if callback is set: " 
              << (manager.ContentUpdateCb ? "Yes" : "No") << std::endl;
    EXPECT_TRUE(static_cast<bool>(manager.ContentUpdateCb));
    ...
    try {
         std::string result = manager.ContentUpdateCb(nullptr, 3, keyId, 30);
         std::cout << "Callback invocation returned: " << result << std::endl;
 ...
 */
TEST(DrmSessionManager, RegisterValidLicenseCallbackSuccess) {
    ...
    DrmSessionManager drmManager(maxDrmSessions, player);
    ...
    DrmSessionManager::LicenseCallback callback = 
        [](DrmHelperPtr drmHelper, int sessionSlot, int &cdmError, GstMediaType streamType, void* metaDataPtr, bool isLicenseRenewal) -> KeyState {
            ...
            cdmError = 0;
            std::cout << "Setting cdmError to " << cdmError << " and returning KeyState::SUCCESS" << std::endl;
            return KeyState::SUCCESS;
        };
    ...
    if (drmManager.AcquireLicenseCb) {
        ...
        int sessionSlot = 1;
        GstMediaType streamType = 0; // Arbitrary value for stream type.
        ...
        std::cout << "Callback returned " << (result == KeyState::SUCCESS ? "KeyState::SUCCESS" : "KeyState::ERROR") 
                  << " with cdmError = " << cdmError << std::endl;
        ASSERT_EQ(result, KeyState::SUCCESS);
    } else {
 ...
 */
TEST(DrmSessionManager, RegisterValidLicenseCallbackError) {
    ...
    if (drmManager.AcquireLicenseCb) {
        ...
        DrmHelperPtr testDrmHelper = nullptr;
        int sessionSlot = 2;
        GstMediaType streamType = 1; // Arbitrary value for stream type.
        void* metaDataPtr = nullptr;
        bool isLicenseRenewal = true;
        KeyState result = drmManager.AcquireLicenseCb(testDrmHelper, sessionSlot, cdmError, streamType, metaDataPtr, isLicenseRenewal);
        std::cout << "Callback returned " << (result == KeyState::ERROR ? "KeyState::ERROR" : "KeyState::SUCCESS") 
                  << " with cdmError = " << cdmError << std::endl;
        ASSERT_EQ(result, KeyState::ERROR);
    } else {
        FAIL() << "AcquireLicenseCb is empty after registering a valid callback.";
    }
 ...
 */
TEST(DrmSessionManager, RegisterEmptyLicenseCallback) {
...
TEST(DrmSessionManager, ValidCallbackRegistration) {
 ...
 */
TEST(DrmSessionManager, RegisteringEmptyCallback) {
 ...
 */
TEST(DrmSessionManager, OverwritingPreviouslyRegisteredCallback) {
...
/**
 * @brief Validate that SetCommonKeyDuration API updates the internal common key duration with a valid positive input
 *
 ...
 * | Variation / Step | Description                                                       | Test Data                                  | Expected Result                                 | Notes         |
 * | :--------------: | ----------------------------------------------------------------- | ------------------------------------------ | ------------------------------------------------ | ------------- |
 * | 01               | Create DrmSessionManager instance using custom constructor        | input1 = 5, input2 = nullptr                 | Instance created successfully                   | Should be successful |
 * | 02               | Set keyDuration variable with a valid positive integer            | keyDuration = 30                           | keyDuration variable is set to 30                | Should be successful |
 * | 03               | Invoke SetCommonKeyDuration API with keyDuration                    | keyDuration = 30                           | Internal common key duration is updated          | Should Pass   |
 * | 04               | Validate the outcome using EXPECT_TRUE assertion                    | return value = true                         | Test assertion passes as EXPECT_TRUE evaluates to true | Should Pass   |
 */
TEST(DrmSessionManager, SetCommonKeyDuration_validPositive) {
    std::cout << "Entering SetCommonKeyDuration_validPositive test" << std::endl;
    
    // Create DrmSessionManager object using custom constructor
    DrmSessionManager drmSessionManager(5, nullptr);
    
    int keyDuration = 30;
    std::cout << "Invoking DrmSessionManager::SetCommonKeyDuration with argument: " << keyDuration << std::endl;
    
    drmSessionManager.SetCommonKeyDuration(keyDuration);
    
    // Log assumed internal state change
    std::cout << "Internal common key duration updated to: " << keyDuration << std::endl;
    
    // Validate that method call completes successfully
    EXPECT_TRUE(true);
    
    std::cout << "Exiting SetCommonKeyDuration_validPositive test" << std::endl;
}
/**
 * @brief Test if DrmSessionManager correctly handles setting a common key duration of zero.
 *
 * This test verifies that when the SetCommonKeyDuration API is invoked with a key duration of zero,
 * the internal state is assumed to be updated accordingly and the method call completes successfully.
 *
 * **Test Group ID:** Basic: 01@n
 * **Test Case ID:** 036@n
 * **Priority:** High@n
 ...
 * | Variation / Step | Description | Test Data | Expected Result | Notes |
 * | :--------------: | ----------- | --------- | ------------- | ----- |
 * | 01 | Instantiate DrmSessionManager object using custom constructor | input1 = 5, input2 = nullptr | Object is instantiated successfully | Should be successful |
 * | 02 | Invoke SetCommonKeyDuration with keyDuration = 0 | keyDuration = 0 | Internal common key duration is updated to 0 without error | Should Pass |
 * | 03 | Validate method execution by checking assertion | Expected assertion: true | EXPECT_TRUE(true) passes confirming successful execution | Should be successful |
 */
TEST(DrmSessionManager, SetCommonKeyDuration_zero) {
    std::cout << "Entering SetCommonKeyDuration_zero test" << std::endl;
    ...
    std::cout << "Exiting SetCommonKeyDuration_zero test" << std::endl;
}
/**
 * @brief Test the behavior of SetCommonKeyDuration when a negative key duration is provided.
 *
 * This test invokes the SetCommonKeyDuration function of the DrmSessionManager class with a negative key duration value (-10) to verify that the API properly handles invalid negative duration values without causing a crash or unexpected behavior.
 *
 * **Test Group ID:** Basic: 01
 * **Test Case ID:** 037
 * **Priority:** High
 ...
 * | Variation / Step | Description                                                     | Test Data                                  | Expected Result                                                         | Notes         |
 * | :----:           | --------------------------------------------------------------- | ------------------------------------------ | ----------------------------------------------------------------------- | ------------- |
 * | 01               | Create a DrmSessionManager instance using a custom constructor    | input1 = 5, input2 = nullptr                 | Instance is created successfully                                        | Should be successful |
 * | 02               | Call SetCommonKeyDuration with a negative key duration value        | input1 = keyDuration, keyDuration = -10     | API properly handles the negative value without causing a crash         | Should Pass   |
 * | 03               | Validate that the method call completes execution without exceptions| output1 = completion status (implicit)       | Method completes execution successfully without throwing any exceptions | Should Pass   |
 */
TEST(DrmSessionManager, SetCommonKeyDuration_negative) {
    std::cout << "Entering SetCommonKeyDuration_negative test" << std::endl;
    ...
    // Log assumed internal state change or handling of negative value.
    std::cout << "Internal handling of negative key duration: " << keyDuration << std::endl;
    
    // Validate that method call completes successfully without crash.
    EXPECT_TRUE(true);
    
    std::cout << "Exiting SetCommonKeyDuration_negative test" << std::endl;
}
/**
 * @brief Verify that DrmSessionManager correctly handles maximum integer key duration.
 *
 * This test invokes DrmSessionManager::SetCommonKeyDuration() with the maximum integer value (INT_MAX) to ensure the API can handle extreme key duration values. It verifies that the method call completes successfully and the internal state is assumed to be updated accordingly.
 *
 * **Test Group ID:** Basic: 01@n
 * **Test Case ID:** 038@n
 * **Priority:** High@n
 ...
 * | Variation / Step | Description | Test Data | Expected Result | Notes |
 * | :----: | --------- | ---------- |-------------- | ----- |
 * | 01 | Create DrmSessionManager object using custom constructor | input1 = 5, input2 = nullptr | Object is created successfully | Should be successful |
 ...
 * | 03 | Invoke SetCommonKeyDuration with keyDuration | keyDuration = INT_MAX, internal update expected | API call completes successfully and internal state is updated | Should Pass |
 * | 04 | Validate API call completes successfully using assertion | ASSERT: EXPECT_TRUE(true) | Assertion passes confirming the expected behavior | Should Pass |
 */
TEST(DrmSessionManager, SetCommonKeyDuration_maxInt) {
    std::cout << "Entering SetCommonKeyDuration_maxInt test" << std::endl;
    
    // Create DrmSessionManager object using custom constructor
    DrmSessionManager drmSessionManager(5, nullptr);
    
    int keyDuration = INT_MAX;
    std::cout << "Invoking DrmSessionManager::SetCommonKeyDuration with argument: " << keyDuration << std::endl;
    ...
    std::cout << "Exiting SetCommonKeyDuration_maxInt test" << std::endl;
}
/**
 * @brief Test to verify that DrmSessionManager::SetCommonKeyDuration correctly handles the minimum integer value for the key duration.
 *
 * This test checks if the API properly processes a negative duration by setting the key duration to INT_MIN.
 * The objective is to ensure that the method call completes without error even for boundary value input.
 *
 * **Test Group ID:** Basic: 01@n
 * **Test Case ID:** 039@n
 * **Priority:** High@n
 ...
 * | Variation / Step | Description | Test Data | Expected Result | Notes |
 * | :----: | --------- | ---------- |-------------- | ----- |
 * | 01 | Log the start of the test execution. | None | "Entering SetCommonKeyDuration_minInt test" message is printed. | Should be successful |
 ...
 * | 04 | Invoke SetCommonKeyDuration with keyDuration. | keyDuration = INT_MIN | API processes the minimum key duration, handling the negative value appropriately. | Should Pass |
 * | 05 | Validate API execution through assertion check. | Assertion: true | EXPECT_TRUE(true) assertion passes confirming successful API call. | Should Pass |
 * | 06 | Log the exit of the test execution. | None | "Exiting SetCommonKeyDuration_minInt test" message is printed. | Should be successful |
 */
TEST(DrmSessionManager, SetCommonKeyDuration_minInt) {
    std::cout << "Entering SetCommonKeyDuration_minInt test" << std::endl;
    
    // Create DrmSessionManager object using custom constructor
    DrmSessionManager drmSessionManager(5, nullptr);
    
    int keyDuration = INT_MIN;
    std::cout << "Invoking DrmSessionManager::SetCommonKeyDuration with argument: " << keyDuration << std::endl;
    
    drmSessionManager.SetCommonKeyDuration(keyDuration);
    
    // Log assumed internal state change or error handling of the negative duration.
    std::cout << "Internal handling of minimum key duration: " << keyDuration << std::endl;
    
    // Validate that method call completes successfully.
    EXPECT_TRUE(true);
    
    std::cout << "Exiting SetCommonKeyDuration_minInt test" << std::endl;
}
/**
 * @brief Validate that calling SetSendErrorOnFailure with true updates the internal configuration without errors
 *
 * This test verifies that when the DrmSessionManager is configured with a valid set of parameters and the
 * SetSendErrorOnFailure method is invoked with the boolean value true, the method executes without throwing
 * any exceptions and updates the internal configuration accordingly.
 *
 * **Test Group ID:** Basic: 01@n
 * **Test Case ID:** 040@n
 * **Priority:** High@n
 ...
 * | Variation / Step | Description                                                     | Test Data                                          | Expected Result                                                         | Notes         |
 * | :--------------: | --------------------------------------------------------------- | -------------------------------------------------- | ----------------------------------------------------------------------- | ------------- |
 * | 01               | Construct DrmSessionManager instance with valid parameters      | maxDrmSessions = 5, player = nullptr               | Instance is successfully constructed without throwing any exceptions  | Should be successful |
 * | 02               | Invoke SetSendErrorOnFailure with sendErrorOnFailure set to true  | sendErrorOnFailure = true                          | Method executes without throwing exceptions and updates configuration   | Should Pass   |
 */
TEST(DrmSessionManager, SetSendErrorOnFailure_true) {
    std::cout << "Entering SetSendErrorOnFailure_true test" << std::endl;
    
    // Create an instance of DrmSessionManager with custom parameters.
    int maxDrmSessions = 5;
    void* player = nullptr;
    std::cout << "Constructed DrmSessionManager with maxDrmSessions = " << maxDrmSessions 
              << " and player = " << player << std::endl;
    ...
    // Invoke the method with sendErrorOnFailure = true.
    bool sendErrorOnFailure = true;
    std::cout << "Invoking DrmSessionManager::SetSendErrorOnFailure with argument: " 
              << (sendErrorOnFailure ? "true" : "false") << std::endl;
    EXPECT_NO_THROW({
        drmSessionManager.SetSendErrorOnFailure(sendErrorOnFailure);
    });
    
    // Log the expected internal configuration update.
    std::cout << "SetSendErrorOnFailure successfully updated internal configuration to: " 
              << (sendErrorOnFailure ? "true" : "false") << std::endl;
    
    std::cout << "Exiting SetSendErrorOnFailure_true test" << std::endl;
}
/**
 * @brief Test verifying that setting sendErrorOnFailure to false does not throw exceptions and updates internal configuration correctly.
 *
 * This test creates an instance of DrmSessionManager with specific parameters, invokes SetSendErrorOnFailure(false),
 * and verifies that no exceptions are thrown during the method call. It also logs the internal configuration change
 * to ensure that the API behaves as expected when disabling error reporting on failure.
 *
 * **Test Group ID:** Basic: 01@n
 * **Test Case ID:** 041@n
 * **Priority:** High@n
 ...
 * | Variation / Step | Description                                                         | Test Data                                 | Expected Result                                   | Notes           |
 * | :----:           | :------------------------------------------------------------------ | :---------------------------------------- | :----------------------------------------------- | :-------------- |
 * | 01               | Instantiate DrmSessionManager with maxDrmSessions and player         | maxDrmSessions = 5, player = nullptr      | DrmSessionManager instance is successfully created | Should be successful |
 * | 02               | Invoke SetSendErrorOnFailure with sendErrorOnFailure set to false     | sendErrorOnFailure = false                | API call does not throw an exception             | Should Pass     |
 * | 03               | Validate internal configuration is updated to reflect the false value  | internal configuration = false            | Internal configuration update is logged correctly | Should be successful |
 */
TEST(DrmSessionManager, SetSendErrorOnFailure_false) {
    std::cout << "Entering SetSendErrorOnFailure_false test" << std::endl;
    ...
    // Invoke the method with sendErrorOnFailure = false.
    bool sendErrorOnFailure = false;
    std::cout << "Invoking DrmSessionManager::SetSendErrorOnFailure with argument: " 
              << (sendErrorOnFailure ? "true" : "false") << std::endl;
    EXPECT_NO_THROW({
        drmSessionManager.SetSendErrorOnFailure(sendErrorOnFailure);
    ...
    std::cout << "Exiting SetSendErrorOnFailure_false test" << std::endl;
}
/**
 ...
 */
TEST(DrmSessionManager, UpdateMaxDRMSessionsPositive) {
    std::cout << "Entering UpdateMaxDRMSessionsPositive test" << std::endl;
    
    DrmSessionManager drmManager(0, nullptr);
 ...
 */
TEST(DrmSessionManager, UpdateMaxDRMSessionsNegative) {
    ...
    DrmSessionManager drmManager(initialMaxSessions, nullptr);
 ...
 */
TEST(DrmSessionManager, Clearing_a_valid_access_token)
{
 ...
 */
TEST(DrmSessionManager, Repeated_calls_to_clearAccessToken)
{
 ...
 */
TEST(DrmSessionManager, ClearDRMSessionForceFalse)
{
    ...
    DrmSessionManager drmManager(5, nullptr);
 ...
 */
TEST(DrmSessionManager, ClearDRMSessionForceTrue)
{
...
TEST(DrmSessionManager, clearFailedKeyIds_noFailedKeys) {
    ...
    DrmSessionManager drmSessionManager(5, dummyPlayer);
 ...
 */
TEST(DrmSessionManager, clearFailedKeyIds_Idempotency) {
    ...
    DrmSessionManager drmSessionManager(10, dummyPlayer);
 ...
 */
TEST(DrmSessionManager, ClearSessionData_NewInstance) {
 ...
 */
TEST(DrmSessionManager, ClearSessionData_MultipleCalls) {
 ...
 */
TEST(DrmSessionManager, ValidDrmSessionCreation) {
    std::cout << "Entering ValidDrmSessionCreation test" << std::endl;
    
    int err = -1;
    // Creating a valid DrmHelperPtr
    std::shared_ptr<DrmHelper> drmHelper = std::make_shared<DrmHelper>();

    // Creating a valid DrmCallbacks pointer
    DrmCallbacks drmCallbacksInstance;
    DrmCallbacks* drmCallbacksPtr = &drmCallbacks;
    int streamType = 1;
 ...
 */
TEST(DrmSessionManager, NullDrmHelperPointer) {
 ...
 */
TEST(DrmSessionManager, NullDrmCallbacksInstance) {
 ...
 */
TEST(DrmSessionManager, NullMetaDataPtr) {
 ...
 */
TEST(DrmSessionManager, InvalidStreamType) {
    ...
    std::cout << "  err initial: " << err << std::endl;
    std::cout << "  drmHelper pointer: " << drmHelper.get() << std::endl;
    std::cout << "  drmCallbacks pointer: " << drmCallbacks << std::endl;
...
/**
 * @brief Validates retrieval of a valid access token from DrmSessionManager with SSL peer verification enabled.
 *
 * This test case verifies that the getAccessToken API in DrmSessionManager returns a non-null access token while
 * updating the token length to a positive value and setting the error code to 0 when SSL peer verification is enabled.
 * Ensuring that these conditions are met indicates that the access token retrieval mechanism functions correctly.
 *
 * **Test Group ID:** Basic: 01@n
 * **Test Case ID:** 059@n
 * **Priority:** High@n
 ...
 * | Variation / Step | Description                                                      | Test Data                                                       | Expected Result                                          | Notes          |
 * | :--------------: | ---------------------------------------------------------------- | --------------------------------------------------------------- | --------------------------------------------------------- | -------------- |
 * | 01               | Create DrmSessionManager object with maxDrmSessions = 5 and player set to nullptr | maxDrmSessions = 5, player = nullptr                             | Object instantiated successfully                         | Should be successful |
 ...
 * | 03               | Invoke getAccessToken API and verify return values                 | bSslPeerVerify = true, tokenLength initially=0, error_code initially=-1 | Access token is not null, tokenLength > 0, error_code equals 0 | Should Pass    |
 */
TEST(DrmSessionManager, ValidAccessTokenRetrievalWithSslPeerVerifyEnabled) {
    std::cout << "Entering ValidAccessTokenRetrievalWithSslPeerVerifyEnabled test" << std::endl;

    // Create object of DrmSessionManager using a custom constructor (maxDrmSessions set to 5, player passed as nullptr)
    std::cout << "Creating DrmSessionManager object with maxDrmSessions = 5 and player = nullptr" << std::endl;
    DrmSessionManager drmManager(5, nullptr);

    // Prepare output parameters
    int tokenLength = 0;
    int error_code = -1;
    bool bSslPeerVerify = true;

    std::cout << "Before invocation: tokenLength = " << tokenLength 
              << ", error_code = " << error_code 
              << ", bSslPeerVerify = " << std::boolalpha << bSslPeerVerify << std::endl;

    // Invoke the getAccessToken method
    std::cout << "Invoking getAccessToken with bSslPeerVerify = true" << std::endl;
    const char* accessToken = drmManager.getAccessToken(tokenLength, error_code, bSslPeerVerify);

    // Debug logs of outputs
    std::cout << "Returned accessToken pointer = " << static_cast<const void*>(accessToken) << std::endl;
    std::cout << "Updated tokenLength = " << tokenLength << std::endl;
    std::cout << "Updated error_code = " << error_code << std::endl;

    // Check the access token is not null
    EXPECT_NE(nullptr, accessToken);
    // Check that tokenLength is greater than 0
    EXPECT_GT(tokenLength, 0);
    // Check that error_code is 0
    EXPECT_EQ(0, error_code);

    std::cout << "Exiting ValidAccessTokenRetrievalWithSslPeerVerifyEnabled test" << std::endl;
}
/**
 * @brief Validate retrieval of a valid access token when SSL peer verification is disabled
 *
 * This test validates that the DrmSessionManager properly initializes and returns a valid access token when invoked with SSL peer verification disabled. It ensures that the returned token pointer is not null, the token length is positive, and the error code is zero, indicating a successful API execution.
 *
 * **Test Group ID:** Basic: 01@n
 * **Test Case ID:** 060@n
 * **Priority:** High@n
 ...
 * | Variation / Step | Description                                                                              | Test Data                                                       | Expected Result                                                                 | Notes        |
 * | :--------------: | ---------------------------------------------------------------------------------------- | --------------------------------------------------------------- | ------------------------------------------------------------------------------- | ------------ |
 * | 01               | Create DrmSessionManager object using custom constructor                                 | maxDrmSessions = 5, player = nullptr                              | Object is created successfully without errors                                  | Should be successful |
 * | 02               | Invoke getAccessToken with bSslPeerVerify set to false                                   | tokenLength = 0, error_code = -1, bSslPeerVerify = false             | Returns a non-null access token pointer, tokenLength > 0, error_code == 0         | Should Pass  |
 */
TEST(DrmSessionManager, ValidAccessTokenRetrievalWithSslPeerVerifyDisabled) {
    std::cout << "Entering ValidAccessTokenRetrievalWithSslPeerVerifyDisabled test" << std::endl;
    ...
    // Invoke the getAccessToken method
    std::cout << "Invoking getAccessToken with bSslPeerVerify = false" << std::endl;
    const char* accessToken = drmManager.getAccessToken(tokenLength, error_code, bSslPeerVerify);
    ...
    std::cout << "Exiting ValidAccessTokenRetrievalWithSslPeerVerifyDisabled test" << std::endl;
}
/**
 ...
 */
TEST(DrmSessionManager, PositivePrimary) {
    ...
    DrmHelperPtr drmHelper = std::make_shared<DrmHelper>();
 ...
 */
TEST(DrmSessionManager, PositiveNonPrimary) {
 ...
 */
TEST(DrmSessionManager, NullDrmHelper) {
 ...
 */
TEST(DrmSessionManager, NullDrmCallbacks) {
 ...
 */
TEST(DrmSessionManager, getSessionMgrState) {
    ...
    DrmSessionManager drmManager(maxDrmSessions, playerPtr);
 ...
 */
TEST(DrmSessionManager, getSlotIdForSession_ValidDrmSession) {
    ...
    DrmSession* session = new DrmSession(keySystem);
 ...
 */
TEST(DrmSessionManager, getSlotIdForSession_NullDrmSession) {
 ...
 */
TEST(DrmSessionManager, hideWatermarkOnDetach_start) {
    ...
    DrmSessionManager drmSessionManager(maxDrmSessions, dummyPlayer);
 ...
 */
TEST(DrmSessionManager, hideWatermarkOnDetach_repeated) {
 ...
 */
TEST(DrmSessionManager, ValidSessionInitialization) {
    ...
    DrmSessionManager dsm(5, nullptr);
    std::cout << "Created DrmSessionManager object with maxDrmSessions=5 and player=nullptr" << std::endl;
...
/**
 * @brief Verify behavior when DrmHelper is nullptr during DRM session initialization.
 *
 * This test verifies that when a null DrmHelper pointer is passed to the initializeDrmSession function of DrmSessionManager,
 * it returns KEY_ERROR_EMPTY_SESSION_ID and sets a non-zero error code. This ensures that the system properly handles invalid input.
 *
 * **Test Group ID:** Basic: 01@n
 * **Test Case ID:** 072@n
 * **Priority:** High@n
 ...
 * | Variation / Step | Description | Test Data | Expected Result | Notes |
 * | :----: | --------- | ---------- |-------------- | ----- |
 * | 01 | Create a DrmSessionManager instance with maxDrmSessions=3 and player=nullptr. | DrmSessionManager(maxDrmSessions=3, player=nullptr) | Object instantiated successfully. | Should be successful |
 ...
 * | 03 | Invoke initializeDrmSession with sessionSlot=0 and initial err=-1. | drmHelper = nullptr, sessionSlot = 0, err = -1 | Returns KEY_ERROR_EMPTY_SESSION_ID and sets err to a non-zero value. | Should Pass |
 * | 04 | Verify assertions: state equals KEY_ERROR_EMPTY_SESSION_ID and err is non-zero. | Output state and err | state == KEY_ERROR_EMPTY_SESSION_ID, err != 0 | Should Pass |
 */
TEST(DrmSessionManager, NullDrmHelper) {
    std::cout << "Entering NullDrmHelper test" << std::endl;
    
    // Creating a valid DrmSessionManager object using default/custom constructor
    DrmSessionManager dsm(3, nullptr);
    std::cout << "Created DrmSessionManager object with maxDrmSessions=3 and player=nullptr" << std::endl;
    
    // DrmHelper pointer is null
    std::shared_ptr<DrmHelper> drmHelper = nullptr;
    std::cout << "Using nullptr for DrmHelper" << std::endl;
    
    int sessionSlot = 0;
    int err = -1;
    std::cout << "Calling initializeDrmSession with sessionSlot = " << sessionSlot << " and initial err = " << err << std::endl;
    
    KeyState state = dsm.initializeDrmSession(drmHelper, sessionSlot, err);
    std::cout << "initializeDrmSession returned KeyState = " << state << std::endl;
    std::cout << "Error code (err) set to = " << err << std::endl;
    
    // Expecting KEY_ERROR_EMPTY_SESSION_ID for null drmHelper.
...
TEST(DrmSessionManager, NegativeSessionSlot) {
    std::cout << "Entering NegativeSessionSlot test" << std::endl;
    
    // Creating a valid DrmSessionManager object using custom constructor
    DrmSessionManager dsm(4, nullptr);
...
/**
 * @brief Validates that initializeDrmSessions correctly initializes DRM sessions without errors
 *
 * This test verifies that when a DrmSessionManager is instantiated with valid parameters,
 * invoking the initializeDrmSessions() method executes without throwing exceptions and properly
 * logs the state of its internal members before and after initialization.
 *
 * **Test Group ID:** Basic: 01@n
 * **Test Case ID:** 074@n
 * **Priority:** High@n
 ...
 * | Variation / Step | Description | Test Data | Expected Result | Notes |
 * | :----: | --------- | ---------- |-------------- | ----- |
 * | 01 | Create a DrmSessionManager instance with test parameters and log its initial state | testMaxDrmSessions = 5, testPlayer = 0x12345, mCustomData = <default>, playerSecInstance = <default> | Instance is successfully created with initial states accessible | Should be successful |
 * | 02 | Invoke initializeDrmSessions() on the instance and log the state before and after the call | No additional input | Method call completes without throwing an exception; internal state updated accordingly | Should Pass |
 * | 03 | Verify the execution flow reaches the SUCCEED() assertion call | No direct input | SUCCEED() is reached confirming the method executed as expected | Should Pass |
 */
TEST(DrmSessionManager, initializeDrmSessions_ValidInitialization) {
    std::cout << "Entering initializeDrmSessions_ValidInitialization test" << std::endl;
    
    // Setup: Create a DrmSessionManager instance with test parameters.
    int testMaxDrmSessions = 5;
    void* testPlayer = reinterpret_cast<void*>(0x12345);
    std::cout << "Creating DrmSessionManager with maxDrmSessions: " << testMaxDrmSessions 
              << " and player pointer: " << testPlayer << std::endl;
              
    DrmSessionManager drmManager(testMaxDrmSessions, testPlayer);
    
    // Log initial state for public members if available.
    std::cout << "Initial mCustomData: " << drmManager.mCustomData << std::endl;
    std::cout << "Initial playerSecInstance pointer: " << drmManager.playerSecInstance << std::endl;
    std::cout << "About to invoke initializeDrmSessions() method" << std::endl;
    
    // Invoke the method under test.
    EXPECT_NO_THROW(
        {
            drmManager.initializeDrmSessions();
        }
    );

    std::cout << "initializeDrmSessions() method invoked" << std::endl;
    
    // For debugging, print the internal public state after initialization.
    std::cout << "After initializeDrmSessions(), mCustomData: " << drmManager.mCustomData << std::endl;
    std::cout << "After initializeDrmSessions(), drmSessionContexts pointer: " << drmManager.drmSessionContexts << std::endl;
    std::cout << "After initializeDrmSessions(), m_drmConfigParam pointer: " << drmManager.m_drmConfigParam << std::endl;
    
    // Since the expected outcome is that the method is invoked without errors,
    // we simply check that no exceptions were thrown.
    SUCCEED();
    
    std::cout << "Exiting initializeDrmSessions_ValidInitialization test" << std::endl;
}
/**
 ...
 */
TEST(DrmSessionManager, notifyCleanup_singleCleanup) {
    ...
    DrmSessionManager drmManager(maxDrmSessions, dummyPlayer);
 ...
 */
TEST(DrmSessionManager, notifyCleanup_multipleCleanup) {
 ...
 */
TEST(DrmSessionManager, RegisterCallbackSuccessfullyWithAllCallbacksProperlySet) {
 ...
 */
TEST(DrmSessionManager, MultipleConsecutiveInvocationsOfRegisterCallback) {
...
/**
 * @brief Verify that releaseLicenseRenewalThreads successfully releases active license renewal threads in a valid DrmSessionManager instance
 *
 * This test creates a DrmSessionManager object with a custom constructor, invokes the releaseLicenseRenewalThreads method, and verifies through internal logging that the active license renewal threads are released. The purpose is to ensure that the method behaves as expected in a normal scenario.
 *
 * **Test Group ID:** Basic: 01@n
 * **Test Case ID:** 079@n
 * **Priority:** High@n
 ...
 * | Variation / Step | Description                                             | Test Data                                           | Expected Result                                                     | Notes           |
 * | :--------------: | ------------------------------------------------------- | --------------------------------------------------- | ------------------------------------------------------------------- | --------------- |
 * | 01               | Create a DrmSessionManager object using the custom constructor with maxDrmSessions and dummyPlayer | maxDrmSessions = 10, dummyPlayer = nullptr            | Object is instantiated successfully                                  | Should be successful |
 * | 02               | Invoke the releaseLicenseRenewalThreads method on the object | drmSessionManager.releaseLicenseRenewalThreads()      | Active license renewal threads are released without error            | Should Pass     |
 * | 03               | Validate that internal state changes reflect thread release   | Internal log message indicating threads have been released | Logs or internal state updates confirm that threads have been released | Should be successful |
 */
TEST(DrmSessionManager, SuccessfullyReleaseActiveLicenseRenewalThreads) {
    std::cout << "Entering SuccessfullyReleaseActiveLicenseRenewalThreads test" << std::endl;

    // Create a DrmSessionManager object using the custom constructor
    int maxDrmSessions = 10;
    void* dummyPlayer = nullptr;
    std::cout << "Creating DrmSessionManager object with maxDrmSessions: " << maxDrmSessions 
              << " and player: " << dummyPlayer << std::endl;
    DrmSessionManager drmSessionManager(maxDrmSessions, dummyPlayer);

    // Log before invoking releaseLicenseRenewalThreads
    std::cout << "Invoking releaseLicenseRenewalThreads method" << std::endl;
    // Call the method to release active license renewal threads
    drmSessionManager.releaseLicenseRenewalThreads();
    std::cout << "Executed releaseLicenseRenewalThreads method" << std::endl;

    // Log internal state changes (assumed internal thread state update)
    std::cout << "Internal thread state updated to indicate threads have been released" << std::endl;

    std::cout << "Exiting SuccessfullyReleaseActiveLicenseRenewalThreads test" << std::endl;
}
/**
 * @brief Verify multiple consecutive calls to releaseLicenseRenewalThreads execute without error and exhibit idempotent behavior.
 *
 * This test verifies that calling the releaseLicenseRenewalThreads method consecutively on a DrmSessionManager instance does not produce any errors or unexpected side effects. The test creates a DrmSessionManager instance and then calls the method twice to ensure that the license renewal threads are properly released each time and that the second call behaves idempotently.
 *
 * **Test Group ID:** Basic: 01@n
 * **Test Case ID:** 080@n
 * **Priority:** High@n
 ...
 * | Variation / Step | Description                                                                               | Test Data                                                   | Expected Result                                                                               | Notes           |
 * | :--------------: | ----------------------------------------------------------------------------------------- | ----------------------------------------------------------- | --------------------------------------------------------------------------------------------- | --------------- |
 * | 01               | Create a DrmSessionManager instance using custom constructor with specified parameters     | maxDrmSessions = 10, dummyPlayer = nullptr                    | DrmSessionManager instance is created successfully                                            | Should be successful |
 * | 02               | Invoke the first call of releaseLicenseRenewalThreads method                              | No input parameter                                          | License renewal threads (if any) are released without errors                                  | Should Pass     |
 * | 03               | Invoke the second call of releaseLicenseRenewalThreads method to test idempotent behavior    | No input parameter                                          | Second call executes without error and confirms idempotent behavior                           | Should Pass     |
 * | 04               | Log overall success message indicating both calls executed successfully                    | No input parameter                                          | Log output confirms that both invocations of releaseLicenseRenewalThreads completed successfully | Should be successful |
 */
TEST(DrmSessionManager, MultipleConsecutiveCallsToReleaseLicenseRenewalThreads) {
    std::cout << "Entering MultipleConsecutiveCallsToReleaseLicenseRenewalThreads test" << std::endl;
    ...
    // First invocation of releaseLicenseRenewalThreads
    std::cout << "Invoking first call of releaseLicenseRenewalThreads method" << std::endl;
    drmSessionManager.releaseLicenseRenewalThreads();
    std::cout << "First call executed, active license renewal threads (if any) have been released." << std::endl;

    // Second invocation of releaseLicenseRenewalThreads
    std::cout << "Invoking second call of releaseLicenseRenewalThreads method" << std::endl;
    drmSessionManager.releaseLicenseRenewalThreads();
    std::cout << "Second call executed, confirming idempotent behavior without errors." << std::endl;

    // Log overall success message indicating both calls executed successfully without exceptions.
    std::cout << "Both invocations of releaseLicenseRenewalThreads completed successfully" << std::endl;

    std::cout << "Exiting MultipleConsecutiveCallsToReleaseLicenseRenewalThreads test" << std::endl;
}
/**
 ...
 */
TEST(DrmSessionManager, StandardLivePlayback) {
 ...
 */
TEST(DrmSessionManager, VODPlayback) {
 ...
 */
TEST(DrmSessionManager, LiveWithFirstFrameSeen) {
 ...
 */
TEST(DrmSessionManager, NegativeCurrentLatency) {
 ...
 */
TEST(DrmSessionManager, NegativeLiveOffsetMs) {
 ...
 */
TEST(DrmSessionManager, ZeroPlaybackSpeed) {
 ...
 */
TEST(DrmSessionManager, ReversePlayback) {
 ...
 */
TEST(DrmSessionManager, ExtremeValues) {
 ...
 */
TEST(DrmSessionManager, SetValidSessionMgrStates) {
 ...
 */
TEST(DrmSessionManager, SetInvalidSessionMgrStates) {
 ...
 */
TEST(DrmSessionManager, PositiveTest_EnableVideoMute_LiveStreaming) {
 ...
 */
TEST(DrmSessionManager, NegativeTest_NegativeLatency) {
 ...
 */
TEST(DrmSessionManager, NegativeTest_NegativeLiveOffset) {
 ...
 */
TEST(DrmSessionManager, EdgeTest_ZeroLatencyOffsetPosition) {
 ...
 */
TEST(DrmSessionManager, NegativeTest_NegativePositionMs) {
 ...
 */
TEST(DrmSessionManager, SetValidVideoWindowSizePositiveDimensions) {
 ...
 */
TEST(DrmSessionManager, SetVideoWindowSizeWithZeroDimensions) {
 ...
 */
TEST(DrmSessionManager, SetVideoWindowSizeWithNegativeWidth) {
 ...
 */
TEST(DrmSessionManager, SetVideoWindowSizeWithNegativeHeight) {
 ...
 */
TEST(DrmSessionManager, SetVideoWindowSizeWithLargeDimensions) {
 ...
 */
TEST(DrmSessionManager, ValidSessionHandleAndProperSystemData)
{
 ...
 */
TEST(DrmSessionManager, BoundaryValuesWithMinimumSessionHandleAndStatus)
{
 ...
 */
TEST(DrmSessionManager, MaximumValuesForSessionHandleAndStatus)
{
 ...
 */
TEST(DrmSessionManager, HandlingEmptySystemDataString)
{
 ...
 */
TEST(DrmSessionManager, UnsupportedStatusCodeScenario)
{
 ...
 */
TEST(DrmSessionManager, SystemDataContainingSpecialOrCorruptCharacters)
{
 ...
 */
TEST(DrmSessionManager, Destructor_ValidDestruction) {
 ...
 */
TEST(DrmSessionManager, RegisterValidLAProfileBegin) {
    ...
    DrmSessionManager drmManager(3, nullptr);
    std::cout << "Created DrmSessionManager object with maxDrmSessions=3 and player=nullptr" << std::endl;
 ...
 */
TEST(DrmSessionManager, RegisterEmptyLAProfileBegin) {
 ...
 */
TEST(DrmSessionManager, RegisterLAProfEnd_ValidCallback)
{
 ...
 */
TEST(DrmSessionManager, RegisterLAProfEnd_EmptyCallback)
{
 ...
 */
TEST(DrmSessionManager, RegisterLAProfEnd_MultiStreamTypes)
{
 ...
 */
TEST(DrmSessionManager, RegisterValidLAProfileErrorCallback)
{
    ...
    DrmSessionManager drmSessionManager(maxDrmSessions, playerPtr);
 ...
 */
TEST(DrmSessionManager, RegisterEmptyLAProfileErrorCallback)
{
 ...
 */
TEST(DrmSessionManager, RegisterValidCallbackNonNull) {
 ...
 * | Variation / Step | Description | Test Data | Expected Result | Notes |
 * | :----: | --------- | ---------- |-------------- | ----- |
 * | 01 | Create a DrmSessionManager instance using a custom constructor. | maxDrmSessions = 5, player = nullptr | Instance is created without exceptions. | Should be successful |
 * | 02 | Define an empty callback using a default constructed std::function. | emptyCallback = default constructed std::function<std::shared_ptr<void>()> | emptyCallback is empty. | Should be successful |
 * | 03 | Register the empty callback using RegisterMetaDataCb. | input callback = emptyCallback | DrmMetaDataCb stored in manager remains empty. | Should Pass |
 * | 04 | Verify that the stored DrmMetaDataCb is empty. | internal state check | DrmMetaDataCb is empty; assertion check passes. | Should Pass |
 */
TEST(DrmSessionManager, RegisterEmptyCallback) {
    std::cout << "Entering RegisterEmptyCallback test" << std::endl;
    ...
    // Define an empty callback using default constructed function
    std::function<std::shared_ptr<void>()> emptyCallback;
    std::cout << "Invoking RegisterMetaDataCb with an empty (null) callback" << std::endl;
    
    // Register the empty callback
    manager.RegisterMetaDataCb(emptyCallback);
    std::cout << "Empty callback registered successfully" << std::endl;
    
    // Verify that the stored callback is empty
    if (!manager.DrmMetaDataCb) {
        std::cout << "DrmMetaDataCb is empty as expected after registering an empty callback" << std::endl;
    } else {
        std::cout << "DrmMetaDataCb is not empty, which is unexpected" << std::endl;
        FAIL() << "Expected DrmMetaDataCb to be empty after registering an empty callback.";
    }
 ...
 */
TEST(DrmSessionManager, RegisterCallbackReturningNullSharedPtr) {
 ...
 */
TEST(DrmSessionManager, ReRegisterCallbackUpdate) {
 ...
 */
TEST(DrmSessionManager, RegisterProfBegin_ValidCallbackTest)
{
 ...
 */
TEST(DrmSessionManager, RegisterProfBegin_EmptyCallbackTest)
{
 ...
 */
TEST(DrmSessionManager, RegisterProfEnd_ValidCallbackSetsFlag) {
 ...
 */
TEST(DrmSessionManager, RegisterProfEnd_UpdateExternalVariable) {
 ...
 */
TEST(DrmSessionManager, RegisterProfEnd_EmptyCallback) {
 ...
 */
TEST(DrmSessionManager, RegisterProfEnd_OverwriteCallback) {
 ...
 */
TEST(DrmSessionManager, RegisterValidProfileErrorCallback) {
 ...
 */
TEST(DrmSessionManager, RegisterEmptyProfileErrorCallback) {
 ...
 */
TEST(DrmSessionManager, OverwriteExistingProfileErrorCallback) {
 ...
 */
TEST(DrmSessionManager, RegisterValidCallbackFunction) {
    ...
    DrmSessionManager drmSessionManager(10, reinterpret_cast<void*>(0x1234));
 ...
 */
TEST(DrmSessionManager, RegisterEmptyCallbackFunction) {
 ...
 */
TEST(DrmSessionManager, OverwriteExistingCallbackFunction) {
 ...
 */
TEST(DrmSessionManager, RegisterValidCallbackWithCapturedNullPointer) {
 ...
 */
TEST(DrmSessionManager, UpdateDRMConfig_AllTrue) {
 ...
 */
TEST(DrmSessionManager, UpdateDRMConfig_AllFalse) {
 ...
 */
TEST(DrmSessionManager, UpdateDRMConfig_Mixed2) {
